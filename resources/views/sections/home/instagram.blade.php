...
<section class="py-32">
    <div class="max-w-7xl mx-auto px-8">
        <div class="flex items-center justify-between mb-16" data-animate="fade-up">
            <div>
                <h2 class="text-4xl font-black font-headline text-primary tracking-tight">{{ $data['title'] ?? 'Latest from Innovesia' }}</h2>
                <p class="text-on-surface-variant mt-2">{{ $data['subtitle'] ?? 'Join our conversation on @innovesia_id' }}</p>
            </div>
            <button class="flex items-center gap-2 px-6 py-2 rounded-full border border-outline-variant text-sm font-bold hover:bg-surface-container-low transition-colors">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6" data-animate="stagger">
            ...
            <div class="group bg-surface-container-lowest rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all">
                ...
                <div class="p-6">
                    <p class="text-sm text-on-surface line-clamp-2 leading-relaxed mb-4">{{ $post['caption'] }}</p>
