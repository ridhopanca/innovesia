    ...
    <div class="max-w-7xl mx-auto px-4 md:px-8">
        <div class="flex flex-col md:flex-row justify-between items-end gap-6 md:gap-8 mb-12 md:mb-20" data-animate="fade-up">
            <div class="max-w-2xl">
                ...
                <span class="text-xs md:text-sm font-bold tracking-widest text-primary/60 font-label uppercase mb-4 block">{{ $data['subtitle'] }}</span>
                @endif
                @if(isset($data['title']))
                <h2 class="text-3xl md:text-4xl lg:text-5xl font-bold font-headline">{{ $data['title'] }}</h2>
                @endif
            </div>
