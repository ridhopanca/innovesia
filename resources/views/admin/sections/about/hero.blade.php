...
<div class="mb-8 bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
    ...
    <form class="section-form p-6 space-y-5" data-id="{{ $section->id }}">
        ...
        <!-- Title -->
        <div>
            <label for="title-{{ $section->id }}" class="text-sm font-semibold text-slate-700 mb-2 block">Judul Utama</label>
        ...
        <div>
            <label for="title_highlight-{{ $section->id }}" class="text-sm font-semibold text-slate-700 mb-2 block">Judul Highlight (bagian berwarna)</label>
            <input type="text" id="title_highlight-{{ $section->id }}" name="title_highlight"
                class="w-full border border-slate-300 p-3 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
                value="{{ $data['title_highlight'] ?? '' }}"
                placeholder="Contoh: Human-Centered Future.">
        </div>

        <!-- Description -->
        <div>
            <label for="description-{{ $section->id }}" class="text-sm font-semibold text-slate-700 mb-2 block">Deskripsi</label>
            <textarea id="description-{{ $section->id }}" name="description"
                class="w-full border border-slate-300 p-3 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
        ...
        <!-- Main Image -->
        <div>
            <label class="text-sm font-semibold text-slate-700 mb-2 block">Gambar Utama</label>
            ...
            <div id="image-url-{{ $section->id }}" class="{{ isset($data['image']) && !str_starts_with($data['image'], 'uploads/') ? '' : 'hidden' }}">
                <input type="text" id="image-{{ $section->id }}" name="image" class="w-full border border-slate-300 p-3 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
            ...
            <div id="image-upload-{{ $section->id }}" class="{{ isset($data['image']) && !str_starts_with($data['image'], 'uploads/') ? 'hidden' : '' }}">
                <input type="file" id="image_file-{{ $section->id }}" name="image_file" class="w-full border border-slate-300 p-3 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all">
            </div>

            @if(isset($data['image']))
            <p class="text-xs text-gray-500 mt-1">Current: {{ $data['image'] }}</p>
            @endif
        </div>
