...
<div class="mb-8 bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
    ...
    <div class="gradient-bg px-6 py-4 flex items-center gap-3 justify-between">
        <div class="flex items-center gap-3">
            <span class="material-icons-outlined text-white text-2xl">lightbulb</span>
    ...
    <form class="section-form p-6 space-y-5" data-id="{{ $section->id }}">
        ...
        <!-- Approaches -->
        <div class="space-y-4">
            <h3 class="font-semibold text-slate-700">Approaches</h3>
            @foreach(($data['approaches'] ?? []) as $index => $approach)
            <div class="border border-slate-200 p-4 rounded-xl bg-slate-50">
                <div class="space-y-3">
                    <div>
                        ...
                        <input type="text" name="approaches[{{ $index }}][icon]"
                            value="{{ $approach['icon'] ?? '' }}"
                            class="w-full border border-slate-300 p-3 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="approaches[{{ $index }}][title]"
                    ...
                    <div>
                        ...
                        <textarea name="approaches[{{ $index }}][description]"
                            class="w-full border border-slate-300 p-3 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
                            rows="3">{{ $approach['description'] ?? '' }}</textarea>
                    </div>
                </div>
            </div>
            @endforeach
